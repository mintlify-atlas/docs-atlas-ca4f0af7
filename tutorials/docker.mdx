...
description: Learn how to deploy Apache Druid using Docker and Docker Compose
---

This quickstart guides you through the steps to download the Apache Druid image from [Docker Hub](https://hub.docker.com/r/apache/druid) and deploy it on a single machine using [Docker](https://www.docker.com/get-started) and [Docker Compose](https://docs.docker.com/compose/). After you finish the initial setup, the cluster will be ready to load data.

Before beginning the quickstart, it is helpful to read the [general Druid overview](/design) and the [ingestion overview](/ingestion), because the tutorials refer to concepts discussed on those pages. It also helps to be familiar with [Docker](https://www.docker.com/get-started).
...
The default `docker-compose.yml` launches eight containers: Zookeeper, PostgreSQL, and six Druid containers based upon the [micro quickstart configuration](/operations/single-server#single-server-reference-configurations-deprecated). Each Druid service is configured to use up to 7 GiB of memory (6 GiB direct memory and 1 GiB heap). However, the quickstart will not use all the available memory.
...
<Tip>
You can modify the value of `DRUID_SINGLE_NODE_CONF` in the Docker `environment` file to use different single-server mode. For example to use the nano quickstart: `DRUID_SINGLE_NODE_CONF=nano-quickstart`.
</Tip>
...
Download this file to the directory created above:

```bash
curl -O https://raw.githubusercontent.com/apache/druid/{{DRUIDVERSION}}/distribution/docker/docker-compose.yml
```
...
<Step title="Download environment file">
The Druid `docker-compose.yml` example uses an [environment file](https://docs.docker.com/compose/environment-variables/#the-env_file-configuration-option) to specify the complete Druid configuration, including the environment variables described in Configuration. This file is named `environment` by default, and must be in the same directory as the `docker-compose.yml` file.

Download the example `environment` file:

```bash
curl -O https://raw.githubusercontent.com/apache/druid/{{DRUIDVERSION}}/distribution/docker/environment
```

The options in this file work well for trying Druid and for using the tutorial.
...
Configuration of the Druid Docker container is done via environment variables set within the container. Docker Compose passes the values from the `environment file` into the container. The variables may additionally specify paths to [the standard Druid configuration files](/configuration) which must be available within the container.
...
- `DRUID_LOG4J` -- set the entire `log4j.xml` configuration file verbatim
...
`cd` into the directory that contains the configuration files:
...
Once the cluster has started, you can navigate to the [web console](/operations/web-console) at [http://localhost:8888](http://localhost:8888). The [Druid router process](/design/router) serves the UI.

![web console](/assets/tutorial-quickstart-01.png)
...
From here you can follow along with the [Quickstart](./index#load-data). For production use, refine your `docker-compose.yml` file to add any additional external service dependencies as necessary.
