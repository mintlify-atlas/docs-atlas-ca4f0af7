...
description: Get started with Apache Druid by installing it locally and running your first ingestion and query
---
...
<Note>
Druid supports a variety of ingestion options. Once you're done with this tutorial, refer to the [Ingestion](/ingestion) page to determine which ingestion method is right for you.
</Note>
...
<Warning>
Java must be available. Either it is on your path, or set one of the `JAVA_HOME` or `DRUID_JAVA_HOME` environment variables. You can run `apache-druid-{{DRUIDVERSION}}/bin/verify-java` to verify Java requirements for your environment.
</Warning>

Before installing a production Druid instance, be sure to review the [security overview](/operations/security-overview). In general, avoid running Druid as root user. Consider creating a dedicated user account for running Druid.
...
<Step title="Understanding Druid storage">
...
After starting the Druid services, open the [web console](/operations/web-console) at [http://localhost:8888](http://localhost:8888).

![web console](/assets/tutorial-quickstart-01.png)

<Note>
It may take a few seconds for all Druid services to finish starting, including the [Druid router](/design/router), which serves the console. If you attempt to open the web console before startup is complete, you may see errors in the browser. Wait a few moments and try again.
</Note>

In this quickstart, you use the the web console to perform ingestion. The MSQ task engine specifically uses the **Query** view to edit and run SQL queries. For a complete walkthrough of the **Query** view as it relates to the multi-stage query architecture and the MSQ task engine, see [UI walkthrough](/operations/web-console).
...
![Data location](/assets/tutorial-quickstart-02.png)
...
![Data sample](/assets/tutorial-quickstart-03.png)

Click **Done**. You're returned to the **Query** view that displays the newly generated query.
</Step>
...
<Step title="Edit the destination datasource name">
...
![Run query](/assets/tutorial-quickstart-04.png)
...
![Query view](/assets/tutorial-quickstart-05.png)
...
- [Druid SQL overview](/querying/sql) or the [Query tutorial](./tutorial-query) to learn about how to query the data you just ingested.
- [Ingestion overview](/ingestion) to explore options for ingesting more data.
- [Tutorial: Load files using SQL](./tutorial-msq-extern) to learn how to generate a SQL query that loads external data into a Druid datasource.
- [Tutorial: Load data with native batch ingestion](./tutorial-batch) to load and query data with Druid's native batch ingestion feature.
- [Tutorial: Load stream data from Apache Kafka](./tutorial-kafka) to load streaming data from a Kafka topic.
- [Extensions](/configuration/extensions) for details on Druid extensions.
